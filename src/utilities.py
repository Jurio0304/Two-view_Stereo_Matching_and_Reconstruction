...
class AccX:
    ...
    @staticmethod
    def compute(prediction_image: np.ndarray, groundtruth_image: np.ndarray, mask_image: np.ndarray = None,
                threshold_disparity: int = 3) -> float:
        ...
        if prediction_image.shape != groundtruth_image.shape:
            raise ValueError(
                "Dimensions of guess (" + prediction_image.shape + ") and groundtruth (" + groundtruth_image.shape + ") do not match.")
...
class IO:
    ...
    def export_image(image: np.ndarray, directory: str, name: str, matching_cost: str, matching_algorithm: str,
        ...
        file_name = str(path) + "_" + matching_cost + "_" + matching_algorithm + "_D" + IO._str_comma(
            max_disparity) + "_R" + IO._str_comma(filter_radius)
