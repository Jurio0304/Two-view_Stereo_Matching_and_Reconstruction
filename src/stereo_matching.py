...
class StereoMatching:
    ...
    def __init__(self, left_image: np.ndarray, right_image: np.ndarray,
        ...
        if right_image.ndim != 2:
            raise ValueError(
                "The right image has to be a grey-scale image with a single channel as its last dimension.")
        if left_image.shape != right_image.shape:
            raise ValueError(
                "Dimensions of left (" + left_image.shape + ") and right image (" + right_image.shape + ") do not match.")
        if max_disparity <= 0:
    ...
    def compute(self) -> None:
        ...
        self._cost_volume = self._matching_cost.compute(self._left_image, self._right_image, self._max_disparity,
                                                        self._filter_radius)
